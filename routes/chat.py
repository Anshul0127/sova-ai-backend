...
def parse_desktop_intent(text: str):
    t = text.lower().strip()
...
async def send_to_agent(user_id: str, command: dict) -> dict:
    ...
    print(f"[AGENT] Active agents: {list(active_agents.keys())}")
    print(f"[AGENT] Looking for user: {user_id}")

    ws = active_agents.get(user_id)
    if not ws:
        print(f"[AGENT] Agent not found for user {user_id}")
        return {"status": "agent_offline"}

    try:
        await ws.send_text(json.dumps(command))
        print(f"[AGENT] Command sent: {command}")
        return {"status": "sent"}
...
def parse_sources(text: str):
    ...
    for line in sources_block.strip().split('\n'):
        ...
        if m:
            ...
            if m2:
                sources.append({"num": m2.group(1), "title": m2.group(2), "url": m2.group(2)})
            ...
            if msg.tool_calls:
                for tool_call in msg.tool_calls:
                    if tool_call.function.name == "web_search":
                        ...
                        search_messages.append({"role": "assistant", "content": f"I searched for: {query}\n\nResults: {result_text}"})
            else:
                break
        except Exception as e:
        ...
        if request.mode == "chat":
            ...
            if desktop_intent and user_id:
                ...
                if result.get("status") == "sent":
                    ...
                    return {"reply": "Your desktop agent isn't running. Start it on your laptop first."}
